...
class Flight(models.Model):
    ...
    def available_seats_dynamic(self):
        """Calculates available seats dynamically considering the booking categories"""
        ...
        booked_first_class = Booking.objects.filter(flight=self, seat_class='First Class').count()
    ...
    def flight_time(self):
        """Calculates the time between departure and arrival in hours and minutes"""
        if self.departure_datetime and self.arrival_datetime:
            time_difference = self.arrival_datetime - self.departure_datetime
